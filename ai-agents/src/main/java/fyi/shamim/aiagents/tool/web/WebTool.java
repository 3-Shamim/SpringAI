...
import fyi.shamim.aiagents.tool.web.record.WebArgs;
import fyi.shamim.aiagents.tool.web.record.WebItem;
...
public class WebTool {
    ...
    public WebTool(WebClient.Builder builder, GcpTokenProvider gcpTokenProvider, AiAgentConfigData aiAgentConfigData) {

        this.webClient = builder
                .baseUrl(aiAgentConfigData.getWebTool().getGoogleVertexSearch().getEndpointBaseUrl())
                .build();
    ...
    @Tool(
            name = "web_search",
            description = "Fetch OWASP/NIST/CWE guidance (allowlisted domains only). Returns title/url/snippet."
    )
    ...
    private List<WebItem> scanCheatSheets(String topic) {
        ...
        String html = this.webClient.get()
                .uri(
                        String.format(
                                "%s%s",
                                this.webToolProperties.getOwasp().getCheatSheetProtocol(),
    ...
    private List<WebItem> googleVertexSearchFiltered(String topic, int topK) {
        ...
        List<Map<String, Object>> results = (List<Map<String, Object>>) googleSearchResponse.getOrDefault(
                "results", List.of()
        );
